...
/**
 * pk_backend_job_finished_delay:
 *
 * We can call into this function if we *know* it's safe.
 **/
static gboolean
pk_backend_job_finished_delay (gpointer data)
{
	...
	job->priv->signal_finished = 0;
	return FALSE;
}

/**
 * pk_backend_job_finished_real:
 **/
static gboolean
pk_backend_job_finished_real (PkBackendJob *job)
{
	...
	g_debug ("adding finished %p to timeout loop", job);
	job->priv->signal_finished = g_timeout_add (PK_BACKEND_FINISHED_TIMEOUT_GRACE,
							pk_backend_job_finished_delay, job);
	g_source_set_name_by_id (job->priv->signal_finished, "[PkBackend] finished");
	return TRUE;
}
...
/**
 * pk_backend_job_finished_cb:
 **/
static gboolean
pk_backend_job_finished_cb (PkBackendJob *job)
{
	pk_backend_job_finished_real (job);
	return FALSE;
}
...
pk_backend_job_finished (PkBackendJob *job)
{
	...
	if (g_thread_self () == job->priv->thread) {
		idle_id = g_idle_add ((GSourceFunc) pk_backend_job_finished_cb, job);
		g_source_set_name_by_id (idle_id, "[PkBackendJob] finished");
	} else {
		pk_backend_job_finished_real (job);
	}
}
