			     ...
			     PkPluginPhase phase)
{
	...
	for (i=0; i<transaction->priv->plugins->len; i++) {
		...
		pk_backend_job_start (transaction->priv->backend, job);
		...
		pk_backend_job_stop (transaction->priv->backend, job);
		plugin->job = NULL;
		plugin->backend = NULL;
	}
out:
...
pk_transaction_finished_cb (PkBackendJob *job, PkExitEnum exit_enum, PkTransaction *transaction)
{
	...
	/* destroy the job */
	pk_backend_job_stop (transaction->priv->backend, transaction->priv->job);
...
pk_transaction_run (PkTransaction *transaction)
{
	...
	/* run the plugins */
	pk_backend_job_start (priv->backend, priv->job);
	pk_transaction_plugin_phase (transaction,
