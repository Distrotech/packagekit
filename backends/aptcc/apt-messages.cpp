...
bool show_errors(PkBackend *backend, PkErrorEnum errorCode, bool errModify)
{
    ...
    if (!errors.str().empty()) {
        pk_backend_job_error_code(backend, errorCode, utf8(errors.str().c_str()));
    }

    if ((errModify) && (!messages.str().empty())) {
        pk_backend_job_message(backend, messageCode, utf8(messages.str().c_str()));
    }
}
...
bool show_warnings(PkBackend *backend, PkMessageEnum message)
{
    ...
    if (!warnings.str().empty()) {
        pk_backend_job_message(backend, message, utf8(warnings.str().c_str()));
    }
}
